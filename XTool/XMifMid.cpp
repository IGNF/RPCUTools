...
XMifMid::~XMifMid()
{
	for (uint32 i = 0; i < m_Repres.size(); i++)
		delete m_Repres[i];
	m_Repres.clear();
}
...
//-----------------------------------------------------------------------------
bool XMifMid::ReadColumns(uint16 nb)
{
	std::string token;
	char buf[1024];
	m_Column.clear();
...
bool XMifMid::Read(const char* filename, bool extract_repres, XError* error)
{
	...
	char sep;
	uint32 num, pos;
	uint32 fill, width, pattern, back, symbol;
...
//-----------------------------------------------------------------------------
bool XMifMid::ReadAttributes(uint32 num, std::vector<std::string>& V)
{
	V.clear();
	if (num == m_nAttPos) {
		V = m_Att;
...
//-----------------------------------------------------------------------------
std::string XMifMid::FindAttribute(uint32 num, std::string att_name)
{
	bool flag = false;
  uint32 i, att_num = 0;
  for (i = 0; i < m_Column.size(); i++)
...
//-----------------------------------------------------------------------------
uint32 XMifMid::MifColor(uint32 c)
{
	uint32 r, g, b;
	byte* ptr = (byte*)&c;
	b = ptr[0];
...
bool XMifMid::Convert(const char* file_in, const char* file_out, XGeodConverter* L, XError* error)
{
	...
	for (uint32 i = 0; i < m_Column.size(); i++)
	...
	while( (!m_In.eof()) && (m_In.good()) ) {
      ...
      m_In >> xi >> yi;
		...
		if (type == "pline") {
			...
			for (uint32 part = 0; part < num; part++) {
				m_In >> nb;
				mif << nb << std::endl;
...
//-----------------------------------------------------------------------------
bool XMifMidMLine2D::Read(std::ifstream* in, uint32 num, XError* error)
{
	m_Pos = in->tellg();
	m_nNumParts = num;
	uint32 nb;
