...
double XGeoVector::Area()
{
  ...
  for (uint32 i = 0; i < NbPart(); i++) {
    if (i < NbPart() - 1)
      ...
      nbpt = NbPt() - Part(i);
    for (uint32 j = Part(i); j < Part(i) + nbpt; j++)
      poly.AddPt(Pt(j));
...
bool XGeoVector::Altitude(std::vector<double>& Alti, double step, bool interpol)
{
	...
	Alti.push_back(zA);
	/*
	for (uint32 i = 1; i < length / step; i++) {
		sample = i * step;
	...
	*/
	...
	for (uint32 i = 1; i < NbPt(); i++) {
		...
		p = (uint32)ceil(dA / step);
		q = (uint32)floor(dB / step);
		if (q >= p) {
			for (uint32 k = p; k <= q; k++) {
				sample = k * step;
...
bool XGeoVector::Position(std::vector<XPt3D>& Pos, double step)
{
	...
	for (uint32 i = 1; i < NbPt(); i++) {
		...
		if (d < step)
			continue;
		nb_inter = (uint32)floor(d / step);
		for (uint32 j = 1; j <= nb_inter; j++) {
			M = A + ((j * step)/d)*(B - A);
			Pos.push_back(M);
		}
	}
...
//-----------------------------------------------------------------------------
XPt2D XGeoVector::LabelPoint(uint16* rot)
{
	if (NbPt() <= 1) {
		if (rot != NULL) *rot = 0;
...
XPt2D XGeoVector::Centroide2D(double dist_min)
{
  ...
  //	return XPt2D(XGEO_NO_DATA, XGEO_NO_DATA);
  double d = dist_min;
  while((d >= 0.01 * Width())&&(d >= 0.01 * Height())) {
    if (IsIn2D(C, d)) {Unload(); return C;}
...
bool XGeoVector::AutoIntersect(XPt2D& M, bool doublons)
{
	...
	for (uint32 i = 0; i < NbPt() - 1; i++) {
		A = Pt(i);
		for (uint32 j = i + step; j < NbPt() - 1; j++) {
			B = Pt(j);
			if (A == B) {
				flag = true;
				M = A;
...
bool XGeoVector::Doublons(XPt2D& M)
{
	...
	for (uint32 i = 0; i < NbPart(); i++) {
		if (i < NbPart() - 1)
			...
			nbpt = NbPt() - Part(i);
		A = Pt(Part(i));
		for (uint32 j = Part(i)+1; j < Part(i) + nbpt; j++) {
			B =  Pt(j);
			if (A == B) {
				flag = true;
				M = A;
...
//-----------------------------------------------------------------------------
uint32  XGeoVector::ZFrame(const XFrame& F, double* zmax, double* zmin, double* zmean)
{
...
//-----------------------------------------------------------------------------
bool XGeoVector::Generalize(std::vector<XPt2D>& T, uint32 nb_max_pt)
{
  T.clear();
  bool loaded = true;
  if (!IsLoaded()) {
...
bool XGeoVector::Generalize(std::vector<XPt2D>& T, double min_angle)
{
  ...
  for (uint32 part = 0; part < NbPart(); part++) {
    Pt_in.clear();
    if (part == NbPart() - 1)
      last_in = NbPt();
    else
      last_in = Part(part + 1);
    for (uint32 j = Part(part); j < last_in; j++)
      Pt_in.push_back(Pt(j));

    for (uint32 j = 0; j < Pt_in.size() - 1; j++) {
      K = Pt_in[j];
      if (j > 0)
        J = Pt_in[j-1];
      else
