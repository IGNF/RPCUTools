...
bool XLambert::Interpol(double lambda, double phi, double* Tx, double* Ty, double* Tz)
{
	...
	uint32 nb_long = (lambda - m_dLongMin) / m_dLongStep;
	uint32 nb_lat = (phi - m_dLatMin) /  m_dLatStep;
...
bool XLambert::LoadGrid(const char* filename, XError* error)
{
	...
	uint32 nb_lig = m_nNbLong * m_nNbLat;
	if (m_Grid != NULL)
		delete[] m_Grid;
...
bool XLambert::WriteGrid(const char* filename, XError* error)
{
	...
	out.write((char*)&m_nNbLat, sizeof(uint32));
	out.write((char*)&m_nNbLong, sizeof(uint32));
...
bool XLambert::ReadGrid(const char* filename, XError* error)
{
	...
	in.read((char*)&m_nNbLat, sizeof(uint32));
	in.read((char*)&m_nNbLong, sizeof(uint32));
