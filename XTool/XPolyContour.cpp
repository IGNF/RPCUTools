...
static void minimax_test(gpc_polygon *subj, gpc_polygon *clip, gpc_op op)
{
  ...
  //       "overlap table creation");
	o_table = new int[subj->num_contours * clip->num_contours];
...
void XPolyContour::gpc_free_polygon(gpc_polygon *p)
{
  int c;
...
void XPolyContour::gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
  ...
  //       * sizeof(int), "contour hole addition");
	extended_hole = new int[p->num_contours + 1];

  /* Create an extended contour array */
  //MALLOC(extended_contour, (p->num_contours + 1)
  //       * sizeof(gpc_vertex_list), "contour addition");
	extended_contour = new gpc_vertex_list[p->num_contours + 1];
  ...
  //       * sizeof(gpc_vertex), "contour addition");
	extended_contour[c].vertex = new gpc_vertex[new_contour->num_vertices];
...
void XPolyContour::gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                      gpc_polygon *result)
{
  sb_tree       *sbtree= NULL;
  ...
  if (result->num_contours > 0)
  {
    ...
    //       * sizeof(int), "hole flag table creation");
		result->hole = new int[result->num_contours];
    ...
    for (poly= out_poly; poly; poly= npoly)
    {
      ...
      if (poly->active)
      {
        ...
        //  "vertex creation");
				result->contour[c].vertex = new gpc_vertex[result->contour[c].num_vertices];
...
void XPolyContour::gpc_free_tristrip(gpc_tristrip *t)
{
  int s;
...
void XPolyContour::gpc_polygon_to_tristrip(gpc_polygon *s, gpc_tristrip *t)
{
  gpc_polygon c;
...
void XPolyContour::gpc_tristrip_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
																		 gpc_tristrip *result)
{
  sb_tree       *sbtree= NULL;
  it_node       *it= NULL, *intersect;
  ...
  if (result->num_strips > 0)
  {
    ...
    for (tn= tlist; tn; tn= tnn)
    {
      ...
      if (tn->active > 2)
      {
        ...
        //       "tristrip creation");
				result->strip[s].vertex = new gpc_vertex[tn->active];
        v= 0;
        if (INVERT_TRISTRIPS)
        {
          lt= tn->v[RIGHT];
...
//-----------------------------------------------------------------------------
// Construction à partir d'un XGeoVector
//-----------------------------------------------------------------------------
bool XPolyContour::Build(XGeoVector* V)
{
  Clear();
  if (V == NULL)
    return false;
...
//-----------------------------------------------------------------------------
// Creation d'un polygone Gpc
//-----------------------------------------------------------------------------
void XPolyContour::CreateGpcPolygon(XPolygone2D* M, gpc_polygon* poly)
{
	poly->num_contours = 1;
	poly->contour = new gpc_vertex_list;
	poly->contour->num_vertices = M->NbPt();
	poly->hole = NULL;
	poly->contour->vertex = new gpc_vertex[M->NbPt()];
	for (uint32 i = 0; i < M->NbPt(); i++) {
		XPt2D A = M->Pt(i);
		poly->contour->vertex[i].x = A.X;
		poly->contour->vertex[i].y = A.Y;
	}
}
...
// Ajout d'un polygone
//-----------------------------------------------------------------------------
void XPolyContour::AddPolygon(XPolygone2D* M)
{
	gpc_polygon poly;
	CreateGpcPolygon(M, &poly);
	gpc_polygon_clip(GPC_UNION, &m_P, &poly, &m_P);
	gpc_free_polygon(&poly);
}

//-----------------------------------------------------------------------------
// Soustraction d'un polygone
//-----------------------------------------------------------------------------
void XPolyContour::SubPolygon(XPolygone2D* M)
{
	gpc_polygon poly;
	CreateGpcPolygon(M, &poly);
	gpc_polygon_clip(GPC_DIFF, &m_P, &poly, &m_P);
	gpc_free_polygon(&poly);
}
...
// XOR d'un polygone
//-----------------------------------------------------------------------------
void XPolyContour::XorPolygon(XPolygone2D* M)
{
	gpc_polygon poly;
	CreateGpcPolygon(M, &poly);
	gpc_polygon_clip(GPC_XOR, &m_P, &poly, &m_P);
	gpc_free_polygon(&poly);
}

//-----------------------------------------------------------------------------
// Intersection d'un polygone
//-----------------------------------------------------------------------------
void XPolyContour::IntPolygon(XPolygone2D* M)
{
	gpc_polygon poly;
	CreateGpcPolygon(M, &poly);
	gpc_polygon_clip(GPC_INT, &m_P, &poly, &m_P);
	gpc_free_polygon(&poly);
}
...
// Operateur de copie
//-----------------------------------------------------------------------------
XPolyContour& XPolyContour::operator=(const XPolyContour& P)
{
	if (this != &P) {
		Clear();
		XPolyContour* ptr = (XPolyContour*)&P;
		Add(ptr);
	}
	return *this;
}
...
//-----------------------------------------------------------------------------
XPolyContour::XPolyContour(const XPolyContour& P)
{
	m_P.contour = NULL;
	m_P.num_contours = 0;
	m_P.hole = NULL;
...
//-----------------------------------------------------------------------------
// Recuperation d'un contour
//-----------------------------------------------------------------------------
bool XPolyContour::GetContour(uint32 num, uint32* nbPt, XPt2D** Pt, bool* hole)
{
	if (num >= m_P.num_contours)
		return false;
	*nbPt = m_P.contour[num].num_vertices;
	*hole = false;
	if (m_P.hole[num])
		*hole = true;
	XPt2D* Tab = new XPt2D[*nbPt];
	if (Tab == NULL)
		return false;
	for (uint32 i = 0; i < *nbPt; i++)
		Tab[i] = XPt2D(m_P.contour[num].vertex[i].x, m_P.contour[num].vertex[i].y);
...
//-----------------------------------------------------------------------------
double XPolyContour::GetContourArea(uint32 num) const
{
	if (num >= m_P.num_contours)
		return 0.;
	if (m_P.contour[num].num_vertices < 3)
		return 0.;
	double area = 0.;
  uint32 i;
  for (i = 0; i < m_P.contour[num].num_vertices - 1; i++)
		area += ( (m_P.contour[num].vertex[i].y + m_P.contour[num].vertex[i+1].y) * 
							(m_P.contour[num].vertex[i+1].x - m_P.contour[num].vertex[i].x) * 0.5);

	// Attention : il faut fermer sur le premier point du polygone
	i = m_P.contour[num].num_vertices - 1;
	area += ( (m_P.contour[num].vertex[i].y + m_P.contour[num].vertex[0].y) * 
						(m_P.contour[num].vertex[0].x - m_P.contour[num].vertex[i].x) * 0.5);
//	if (m_P.hole[num])
//		area *= -1.;
	return area;
}
...
//-----------------------------------------------------------------------------
// Surface du PolyContour
//-----------------------------------------------------------------------------
double XPolyContour::Area() const
{
	double area = 0;
	for (uint32 i = 0; i < m_P.num_contours; i++)
		area += GetContourArea(i);
	return area;
}

//-----------------------------------------------------------------------------
// Rectangle englobant du PolyContour
//-----------------------------------------------------------------------------
XFrame XPolyContour::Frame() const
{
	XFrame F;
	if (m_P.num_contours < 1)
		return F; 
	if (m_P.contour[0].num_vertices < 1)
		return F;
	F.Xmin = F.Xmax = m_P.contour[0].vertex[0].x;
	F.Ymin = F.Ymax = m_P.contour[0].vertex[0].y;
	for (uint32 i = 0; i < m_P.num_contours; i++) 
		for (uint32 j = 0; j < m_P.contour[i].num_vertices; j++) {
			F.Xmin = XMin(F.Xmin, m_P.contour[i].vertex[j].x);
			F.Ymin = XMin(F.Ymin, m_P.contour[i].vertex[j].y);
			F.Xmax = XMax(F.Xmax, m_P.contour[i].vertex[j].x);
			F.Ymax = XMax(F.Ymax, m_P.contour[i].vertex[j].y);
		}
	return F;
}

//-----------------------------------------------------------------------------
// Trie des contours en fonction de la surface
//-----------------------------------------------------------------------------
void XPolyContour::Sort()
{
	if (m_P.num_contours < 2)
		return;
	double* area = new double[m_P.num_contours];
  uint32 i;
  for (i = 0; i < m_P.num_contours ; i++)
		area[i] = GetContourArea(i);

	gpc_vertex_list tmp_contour;
	int tmp_hole;
	double tmp_area;
	
	for (i = 0; i < m_P.num_contours ; i++) 
		for (uint32 j = 0; j < m_P.num_contours - 1; j++) {
			if (area[j+1] > area[j]) {
				tmp_area = area[j+1];
				tmp_hole = m_P.hole[j+1];
				tmp_contour = m_P.contour[j+1];

				area[j+1] = area[j];
				m_P.hole[j+1] = m_P.hole[j];
				m_P.contour[j+1] = m_P.contour[j];

				area[j] = tmp_area;
				m_P.hole[j] = tmp_hole;
				m_P.contour[j] = tmp_contour;
			}
		}
...
// Operateurs logiques
//-----------------------------------------------------------------------------
bool operator==(XPolyContour A, XPolyContour B)
{
	XPolyContour tmp;
	tmp.Add(&A);
	tmp.Xor(&B);
...
//-----------------------------------------------------------------------------
// Test si un polygone est inclus dans un autre
//-----------------------------------------------------------------------------
bool XPolyContour::IsIn(XPolyContour* M)
{
	XPolyContour tmp = *this;
	tmp.Add(M);
	return (*this == tmp);
}

bool XPolyContour::IsIn(XPolygone2D* M)
{
	XPolyContour tmp;
	tmp.AddPolygon(M);
	return IsIn(&tmp);
...
bool XPolyContour::IsIn(XPt2D* P, double d)
{
	XPolygone2D poly;
	poly.AddPt(P->X - d, P->Y - d);
	poly.AddPt(P->X + d, P->Y - d);
	poly.AddPt(P->X + d, P->Y + d);
	poly.AddPt(P->X - d, P->Y + d);
	return IsIn(&poly);
...
//-----------------------------------------------------------------------------
bool XPolyContour::XmlRead(XParserXML* parser, uint32 num, XError* error)
{
	XParserXML poly = parser->FindSubParser("/poly_contour", num);
	if (poly.IsEmpty())
		return XErrorError(error, "XPolyContour::XmlRead", XError::eBadFormat);
	...
	gpc_vertex_list V;
	while(true){
		contour = poly.FindSubParser("/poly_contour/contour", nb);
		if (contour.IsEmpty())
			break;
		hole = contour.ReadNodeAsInt("/contour/hole");
		V.num_vertices = contour.ReadNodeAsInt("/contour/nb_pt");
		V.vertex  = new gpc_vertex[V.num_vertices];
		if (V.vertex == NULL)
			return XErrorError(error, "XPolyContour::XmlRead", XError::eAllocation);

		std::vector<double> X, Y;
		contour.ReadArrayNodeAsDouble("/contour/x", &X);
		contour.ReadArrayNodeAsDouble("/contour/y", &Y);
		if ( (V.num_vertices != X.size()) || (V.num_vertices != Y.size()))
			return XErrorError(error, "XPolyContour::XmlRead", XError::eBadData);

		for (uint32 i = 0; i < V.num_vertices; i++) {
			V.vertex[i].x = X[i];
			V.vertex[i].y = Y[i];
		}

		gpc_add_contour(&m_P, &V, hole);
		delete[] V.vertex;
...
//-----------------------------------------------------------------------------
bool XPolyContour::XmlWrite(std::ostream* out)
{
	int prec = out->precision(2);						// Sauvegarde des parametres du flux
	std::ios::fmtflags flags = out->setf(std::ios::fixed);

	*out << "<poly_contour>" << std::endl;
	for (uint32 i = 0; i < NbContour(); i++) {
		*out << "<contour>" << std::endl;
		*out << "<hole> " << m_P.hole[i] << " </hole>" << std::endl;
		*out << "<nb_pt> " <<  m_P.contour[i].num_vertices << " </nb_pt>" << std::endl;
		for (uint32 j = 0; j < m_P.contour[i].num_vertices; j++)
			*out<< "<x> " << m_P.contour[i].vertex[j].x << " </x>"
					<< "<y> " << m_P.contour[i].vertex[j].y << " </y>" << std::endl;
		*out << "</contour>" << std::endl;
	}
	*out << "</poly_contour>" << std::endl;
...
//-----------------------------------------------------------------------------
// Ecriture de la géométrie dans un fichier texte au format Géoconcept
//-----------------------------------------------------------------------------
bool XPolyContour::ExportGeoconcept(std::ostream* out)
{
	char Sep = '\t';
	XPt2D Pi,Pj;
	...
	for (uint32 i = 0; i < NbContour(); i++) {
		Pi.X = m_P.contour[i].vertex[0].x;
		Pi.Y = m_P.contour[i].vertex[0].y;
		*out << Sep << Pi.X << Sep << Pi.Y << Sep << m_P.contour[i].num_vertices - 1;	
		for (uint32 j = 1; j < m_P.contour[i].num_vertices; j++){
			Pj.X = m_P.contour[i].vertex[j].x;
			Pj.Y = m_P.contour[i].vertex[j].y;
			*out << Sep << Pj.X - Pi.X << Sep << Pj.Y - Pi.Y;
			Pi = Pj;
		}
		if ((i == 0) && (NbContour() > 1))
			*out << Sep << NbContour()-1 ;
...
//-----------------------------------------------------------------------------
// Ecriture de la géométrie dans un fichier texte au format MIF/MID
//-----------------------------------------------------------------------------
bool XPolyContour::ExportMifMid(std::ostream* out)
{
  XPt2D Pi, Pj;
  ...
  *out << "REGION " << NbContour() << std::endl;
  for (uint32 i = 0; i < NbContour(); i++) {
    *out << m_P.contour[i].num_vertices << std::endl;
    for (uint32 j = 0; j < m_P.contour[i].num_vertices; j++)
      *out << m_P.contour[i].vertex[j].x << " " << m_P.contour[i].vertex[j].y << std::endl;
  }
...
//-----------------------------------------------------------------------------
// Export sous forme de table pour une BD relationnelle
//-----------------------------------------------------------------------------
bool XPolyContour::ExportDatabase(std::ostream* out, const char* header, char sep)
{
	int prec = out->precision(2);						// Sauvegarde des parametres du flux
	std::ios::fmtflags flags = out->setf(std::ios::fixed);
	...
	for (uint32 i = 0; i < NbContour(); i++) {
		for (uint32 j = 0; j < m_P.contour[i].num_vertices; j++){
			*out << header << sep << i << sep << j << sep 
				<< m_P.contour[i].vertex[j].x << sep << m_P.contour[i].vertex[j].y << std::endl;
		}
	}
