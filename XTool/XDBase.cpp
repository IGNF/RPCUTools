...
// Constructeur
//-----------------------------------------------------------------------------
XDBaseFileHeader::XDBaseFileHeader(uint32 nbRecord, uint32 recordSize, uint32 nb_field)
{
	m_cVersion = 0x03;
...
//-----------------------------------------------------------------------------
XDBaseFieldDescriptor::XDBaseFieldDescriptor(const char* name, char t, byte size, byte dec)
{
	m_strName = name;
	m_nLength = size;
	m_nDecCount = dec;
...
bool XDBaseFieldDescriptor::Read(std::ifstream* in, XError* error)
{
	...
	in->read(buf, 4);
	in->read(buf, 1);
	m_nLength = ((byte*)buf)[0];
	in->read(buf, 1);
	m_nDecCount = ((byte*)buf)[0];
	in->read(buf, 14);
...
bool XDBaseFile::ReadHeader(const char* filename, XError* error)
{
  ...
  if (!m_Header.Read(in, error))
	...
	uint32 nb_field = (m_Header.HeaderSize() - 32) / 32;
	m_FieldDesc.clear();
	for (uint32 i = 0; i < nb_field; i++) {
...
//-----------------------------------------------------------------------------
bool XDBaseFile::ReadRecord(uint32 num, std::vector<std::string>& V, XError* error)
{
	V.clear();
  std::ifstream* in = m_In.IStream();
  if (in == NULL)
    return  false;
  uint32 pos = (uint32)m_Header.HeaderSize() + num * (uint32)m_Header.RecordSize() + 1L;
  if (pos >= 2147483647L){ // Fichier > 2GB
    ...
    in->read(buf, desc.Length());
		...
		default:
			buf[desc.Length()] = '\0';
			for (uint32 j = desc.Length() - 1; j > 0; j--)
				if (buf[j] == ' ')
					buf[j] = '\0';
				else
					break;
...
//-----------------------------------------------------------------------------
bool XDBaseFile::GetFieldDesc(uint32 index, std::string& name, XDBaseFieldDescriptor::eType& type,
															uint16& length, byte& dec)
{
...
// Ajout d'un champ
//-----------------------------------------------------------------------------
bool XDBaseFile::AddField(const char* name, char type, byte size, byte dec)
{
...
//-----------------------------------------------------------------------------
bool XDBaseFile::SetNbRecord(uint32 nb)
{
	uint32 size = 1;
	for (uint32 i = 0; i < m_FieldDesc.size(); i++)
...
//-----------------------------------------------------------------------------
bool XDBaseFile::UpdateRecord(uint32 num, std::vector<std::string>& V, XError* error)
{
	char buf[256];
	short s;
	int n;
