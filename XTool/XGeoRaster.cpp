...
void XGeoRasterContext::Clear()
{
	...
	if (m_Max != NULL)
		delete[] m_Max;
	if (m_Lut != NULL) {
		for (uint16 i = 0; i < m_nNbChannel; i++)
			if (m_Lut[i] != NULL)
				delete[] m_Lut[i];
		delete[] m_Lut;
	}
...
void XGeoRasterContext::Clone(XGeoRasterContext* clone)
{
  ...
  ::memcpy( m_Lut[0], clone->m_Lut[0], 256 * sizeof(byte));
}

//-----------------------------------------------------------------------------
// Fixe le nombre de canaux geres par le context
//-----------------------------------------------------------------------------
bool XGeoRasterContext::NbChannel(uint16 nb)
{
	Clear();
	if (nb == 0) 
		return true;
...
//-----------------------------------------------------------------------------
bool XGeoRasterContext::Channel(uint16 nb, double gamma, byte min, byte max)
{
	if (nb >= m_nNbChannel)
		return false;
	m_Gamma[nb] = gamma;
  ...
  for (i = 0; i < min; i++)
		m_Lut[nb][i] = 0;
	for (i = min; i < max; i++)
		m_Lut[nb][i] = (byte)(pow((double)(i - min) / (double)(max - min), gamma) * 255.0);
	for (i = max; i < 256; i++)
		m_Lut[nb][i] = 255;
...
//-----------------------------------------------------------------------------
bool XGeoRasterContext::Channel(uint16 nb, double* gamma, byte* min, byte* max, byte** lut)
{
	if (nb >= m_nNbChannel)
		return false;
	*gamma = m_Gamma[nb];
...
//-----------------------------------------------------------------------------
byte* XGeoRasterContext::Lut(uint16 nb)
{
	if (nb >= m_nNbChannel)
		return NULL;
	return m_Lut[nb];
...
// Fixe la palette
//-----------------------------------------------------------------------------
void XGeoRasterContext::Palette(byte* pal)
{
  ...
  if (m_Palette == NULL) {
    m_Palette = new byte[256 * 3];
    if (m_Palette == NULL) return;
  }
  ::memcpy(m_Palette, pal, 256 * 3 * sizeof(byte));
}
...
//-----------------------------------------------------------------------------
bool XGeoRasterHisto::NbChannel(uint16 nb)
{
	Clear();
	if (nb == 0) 
		return true;
...
//-----------------------------------------------------------------------------
bool XGeoRasterHisto::Channel(uint16 nb, double* H)
{
	if (nb >= m_nNbChannel)
		return false;
...
//-----------------------------------------------------------------------------
bool XGeoRasterHisto::Analyze(uint16 nb, uint16& min, uint16& max, uint16& nblevel,
															uint16& nbhole, uint16& holemax, double& satur, double holesize)
{
	if (nb >= m_nNbChannel)
		return false;
...
//-----------------------------------------------------------------------------
bool XGeoRasterHisto::Write(uint16 nb, std::ostream& out)
{
	if (nb >= m_nNbChannel)
		return false;
...
bool XGeoRaster::Gamma(double gamma)
{
	...
	byte min, max;
	byte* LUT;
	for (uint16 num = 0; num < m_Context->NbChannel(); num++) {
...
//-----------------------------------------------------------------------------
bool XGeoRaster::AnalyzeHisto(uint16 nb, uint16& min, uint16& max, uint16& nblevel, 
															uint16& nbhole, uint16& holemax, double& satur, double holesize)
{ 
	if (m_Histo == NULL) {
...
// Ecriture d'un histogramme dans un flux
//-----------------------------------------------------------------------------
bool XGeoRaster::WriteHisto(uint16 nb, std::ostream& out)
{
	if (m_Histo == NULL)
		return false;
...
//-----------------------------------------------------------------------------
bool XGeoRaster::Pix2Ground(uint32 u, uint32 v, double& x, double& y)
{
  if ((u >= m_nW)||(v >= m_nH))
    return false;
...
//-----------------------------------------------------------------------------
bool XGeoRaster::Ground2Pix(double x, double y, uint32& u, uint32& v)
{
