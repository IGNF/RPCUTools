...
bool XShapefileRecordHeader::Read(std::ifstream* in, XError* error)
{
	...
	endian.Read(in, false, &m_nNumber, 4);
	endian.Read(in, false, &m_nLength, 4);
...
bool XShapefileRecordHeader::Write(std::ofstream* out, XError* error)
{
	...
	endian.Write(out, false, &m_nNumber, 4);
	endian.Write(out, false, &m_nLength, 4);
...
// Taille en octets
//-----------------------------------------------------------------------------
uint32 XShapefileRecord::ByteSize() const
{
	switch(m_eShapeType) {
		case shpNullShape : 
			return 4;
...
bool XShapefileRecord::ReadMultiPatch(std::ifstream* in, XEndian* endian, XShapefileRecordHeader* header, XError* error)
{
	...
	if (!ReadParts(in, endian, error))
		return false;
	for (uint32 i = 0; i < m_nNumParts; i++)
