...
protected:
  ...
  bool AnalyzeLineString(std::istream* in, XFrame* F, uint32* nbPt, XPt *P = NULL);
  bool AnalyzePolygon(std::istream* in, XFrame* F, uint32* nbPt, uint32* nbPart,
                      XPt *P = NULL, int *Parts = NULL);
  bool AnalyzeMultiPolygon(std::istream* in, XFrame* F, uint32* nbPt, uint32* nbPart,
                      XPt *P = NULL, int *Parts = NULL);
  bool AnalyzeGeometry(std::istream* in, std::string &type, XFrame *F, uint32 *nbPt, uint32 *nbPart,
                       XPt *P = NULL, int *Parts = NULL);
...
public:
  ...
  bool ReadFeatureGeom(uint32 pos, uint32 nbPt, XPt* P, uint32 nbPart = 1, int *Parts = NULL);

  bool ReadAttributes(uint32 pos, std::vector<std::string>& V);
...
public :
  XGeoJsonVector() { m_File = NULL;}
  XGeoJsonVector(XGeoJson* file, uint32 pos) { m_File = file; m_Pos = pos;}

  inline XGeoJson* GeoJson() const { return m_File;}
  inline uint32 Pos() const { return m_Pos;}
  void GeoJson(XGeoJson* geojson, uint32 pos) { m_File = geojson; m_Pos = pos;}
...
public:
  ...
  XGeoJsonPoint2D(XGeoJson* file, uint32 pos, XFrame* F) : XGeoJsonVector(file, pos) { m_Frame = *F;}
...
public:
  ...
  XGeoJsonMPoint2D(XGeoJson* file, uint32 pos, XFrame* F, uint32 *nbPt) : XGeoJsonVector(file, pos)
...
public:
  XGeoJsonMLine2D(XGeoJson* file, uint32 pos, XFrame* F, uint32 *nbPt, uint32 *nbPart) : XGeoJsonVector(file, pos)
...
public:
  ...
  XGeoJsonLine2D(XGeoJson* file, uint32 pos, XFrame* F, uint32 *nbPt) : XGeoJsonVector(file, pos)
    { m_Frame = *F; m_nNumPoints = *nbPt;}
...
public:
  ...
  XGeoJsonMPoly2D(XGeoJson* file, uint32 pos, XFrame* F, uint32 *nbPt, uint32 *nbPart) : XGeoJsonVector(file, pos)
    { m_Frame = *F; m_nNumPoints = *nbPt; m_nNumParts = *nbPart;}
...
public:
  ...
  XGeoJsonPoly2D(XGeoJson* file, uint32 pos, XFrame* F, uint32 *nbPt) : XGeoJsonVector(file, pos)
    { m_Frame = *F; m_nNumPoints = *nbPt;}
