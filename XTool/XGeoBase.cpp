...
{
	for (uint32 i = 0; i < m_Layer.size(); i++)
...
//-----------------------------------------------------------------------------
uint32 XGeoBase::Find(std::vector<XGeoVector*>* T, XPt2D& P, double dist)
{
	XGeoLayer* layer = NULL;
	XGeoClass* classe;
	XGeoVector* vector;
	...
	for (uint32 i = 0; i < m_Layer.size(); i++) {
		...
		for (uint32 j = 0; j < layer->NbClass(); j++) {
			...
			for (uint32 k = 0; k < classe->NbVector(); k++) {
...
//-----------------------------------------------------------------------------
uint32 XGeoBase::Find(std::vector<XGeoVector*>* T, XFrame& F)
{
	XGeoLayer* layer = NULL;
	XGeoClass* classe;
	XGeoVector* vector;
...
//-----------------------------------------------------------------------------
uint32 XGeoBase::SortClass()
{
	if (NbLayer() < 1)
		return 0;
	m_Class.clear();
	for (uint32 i = 0; i < NbLayer(); i++) {
...
// Renvoi une classe
//-----------------------------------------------------------------------------
XGeoClass* XGeoBase::Class(uint32 i)
{
	if (i < m_Class.size())
		return m_Class[i];
	return NULL;
		...
		for (uint32 j = 0; j < layer->NbClass(); j++) {
			...
			for (uint32 k = 0; k < classe->NbVector(); k++) {
				...
				if ((vector->TypeVector() != XGeoVector::Raster)&&(vector->TypeVector() != XGeoVector::DTM))
					continue;
				resol = XMin (resol, ((XGeoRaster*)vector)->Resolution());
			}
		}
	}
	return resol;
...
XGeoVector* XGeoBase::FindTopRasterObject(XFrame* F)
{
	...
	for (uint32 i = 0; i < NbClass(); i++) {
		...
		for (uint32 j = 0; j < classe->NbVector(); j++) {
...
//-----------------------------------------------------------------------------
uint32 XGeoBase::NbObjectFrame(XFrame* F, bool only_visible)
{
	XGeoLayer* layer = NULL;
	XGeoClass* classe;
	XGeoVector* vector;
...
double XGeoBase::Z(const XPt2D& P)
{
  ...
  XGeoVector* V;
  double z;
  for (uint32 i = 0; i < layer->NbClass(); i++) {
...
//-----------------------------------------------------------------------------
bool XGeoBase::Z(const XPt2D& TL, float* T, double delta, uint32 w, uint32 h)
{
  XGeoLayer* layer = Layer("DTM");
  if (layer == NULL)
    return false;
...
void XGeoBase::RemoveClass(const char* layername, const char* classname)
{
  ...
  for (uint32 i = 0; i < C->NbVector(); i++) {
    ...
    for (uint32 k = 0; k < m_Map.size(); k++) {
      XGeoMap* map = m_Map[k];
      for (uint32 p = 0; p < map->NbMap(); p++) {
  ...
  for (uint32 i = 0; i < C->NbVector(); i++) {
    XGeoVector* V = C->Vector(i);
    for (uint32 p = 0; p < m_Class.size(); p++) {
	...
	XGeoLayer* layer = NULL;
	for (uint32 k = 0; k < m_Layer.size(); k++)
