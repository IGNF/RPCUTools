...
public:
  ...
  virtual void Editable(bool flag) { if (flag) m_State |= sEditable; else m_State &= ~sEditable;}
	...
	inline double Width() { return m_Frame.Width();}
	inline double Height() { return m_Frame.Height();}
...
//	virtual inline XPt2D Centroide() const { return XPt2D(XGEO_NO_DATA, XGEO_NO_DATA);}
  ...
  virtual void Filename(std::string name) { ;}
  virtual std::string Filename() { return "";}
	...
	virtual bool XmlRead(XParserXML* parser, uint32 num = 0, XError* error = NULL) {return true;}
	virtual bool XmlWrite(std::ostream* out) { return true;}


	virtual bool IsNear2D(const XPt2D& P, double dist) { return false;}

	virtual bool WriteHtml(std::ostream* out) { return false;}
