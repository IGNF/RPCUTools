...
//-----------------------------------------------------------------------------
bool XGeoJson::AnalyzeLineString(std::istream* in, XFrame* F, uint32* nbPt, XPt* P)
{
  char sep;
  double x, y, z;
  *F = XFrame();
...
//-----------------------------------------------------------------------------
bool XGeoJson::AnalyzePolygon(std::istream* in, XFrame* F, uint32* nbPt, uint32* nbPart,
                               XPt* P, int *Parts)
{
  char sep;
  double x, y, z;
...
//-----------------------------------------------------------------------------
bool XGeoJson::AnalyzeMultiPolygon(std::istream* in, XFrame* F, uint32* nbPt, uint32* nbPart,
                                    XPt* P, int *Parts)
{
  char sep;
  double x, y, z;
...
bool XGeoJson::AnalyzeGeometry(std::istream* in, std::string &type, XFrame* F,
                               uint32* nbPt, uint32* nbPart, XPt* P, int *Parts)
{
  char sep;
  *in >> sep;
  if (sep != '{')
...
//-----------------------------------------------------------------------------
bool XGeoJson::ReadFeatureGeom(uint32 pos, uint32 nbPt, XPt* P, uint32 nbPart, int* Parts)
{
...
//-----------------------------------------------------------------------------
bool XGeoJson::ReadAttributes(uint32 pos, std::vector<std::string>& V)
{
  if (pos >= 2147483647L) {// Fichier > 2GB
    m_In.seekg(2147483647L);
    m_In.seekg((pos - 2147483647L), std::ios_base::cur);
